...
class WHILL_Data(Dataset):
    ...
    def __init__(self, data_root, conditions, config):
        ...
        for condition in conditions:
            ...
            preload_file = os.path.join(sub_root, 'aim_mt_seq'+str(self.seq_len)+'_pred'+str(self.pred_len)+'_rp1'+str(self.rp1_close)+'_maf'+str(self.config.n_buffer*self.data_rate)+'.npy')
            ...
            if not os.path.exists(preload_file):
                ...
                for route in routes:
                    ...
                    for i in range(0, len(files)-(self.seq_len-1)-(self.pred_len*self.data_rate)): #kurangi sesuai dengan jumlah sequence dan wp yang akan diprediksi
                        ...
                        preload_linear_velo.append(np.mean(np.abs(meta_current['whill_LR_wheel_angular_velo']))*self.config.wheel_radius)
                ...
                preload_dict['linear_velo'] = preload_linear_velo
            ...
            self.dep += preload_dict.item()['dep']
            ...
            self.linear_velo += preload_dict.item()['linear_velo']
    ...
    def __getitem__(self, index):
        ...
        for i in range(0, self.seq_len):
            ...
            depx = np.nan_to_num(crop_matrix(np.load(seq_deps[i]), D3=False, resize=self.config.scale, crop=self.config.crop_roi), nan=0.0, posinf=39.99999, neginf=0.2) / self.config.max_depth #normalisasi 0 - 1
            data['deps'].append(torch.from_numpy(np.expand_dims(np.array(depx), axis=0)))#tambahkan 1 channel depan buat torch
        ...
        data['linear_velo'] = self.linear_velo[index]
